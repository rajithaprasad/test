...
interface LoanDetailsSection5Props {
  formData: FormData;
...
const LoanDetailsSection5 = ({ formData, updateFormData, onNext, onPrevious }: LoanDetailsSection5Props) => {

  const handlePropertyValueChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const propertyValue = parseInt(e.target.value) || 0;
    const totalCostBasis = propertyValue + (formData.rehabCost || 0);
    const marginOnImprovement = formData.borrowerARV ? (formData.borrowerARV / totalCostBasis) * 100 : 0;
    const loanAmount = propertyValue * (formData.requestedLTAIV || 0) / 100;
  ...
  const handleRehabCostChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const rehabCost = parseInt(e.target.value) || 0;
    const totalCostBasis = (formData.propertyValue || 0) + rehabCost;
    const marginOnImprovement = formData.borrowerARV ? (formData.borrowerARV / totalCostBasis) * 100 : 0;
  ...
  const handleBorrowerARVChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const totalCostBasis = (formData.propertyValue || 0) + (formData.rehabCost || 0);
  ...
  const handleRequestedLTAIVChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const requestedLTAIV = parseInt(e.target.value) || 0;
    const loanAmount = (formData.propertyValue || 0) * requestedLTAIV / 100;
    ...
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
     <ProgressIndicator
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600">Section 5: Fix & Flip - Refinance</p>
      </div>

      <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
        <h3 className="text-xl font-semibold text-blue-800 mb-6">Loan Details - Section 5</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="floating-input relative">
            <input
              ...
              onChange={handleRehabCostChange}
              placeholder=" "
              className="w-full p-4 border-2 border-gray-200 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg"
            />
            <label htmlFor="rehabCost" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              Rehab Cost ($)
            </label>
          </div>
          ...
          <div className="floating-input relative">
            <input
              type="number"
              id="requestedLTAIV"
              value={formData.requestedLTAIV || ''}
              onChange={handleRequestedLTAIVChange}
              placeholder=" "
              className="w-full p-4 border-2 border-gray-200 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg"
            />
            <label htmlFor="requestedLTAIV" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              LTAIV Requested (%)
            </label>
          </div>
          ...
          <div className="floating-input relative">
            <input
              ...
              className="w-full p-4 border-2 border-gray-200 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg"
            />
            <label htmlFor="maxLTAIV" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              Maximum LTAIV (%)
            </label>
          </div>
        </div>
...
export default LoanDetailsSection5;
