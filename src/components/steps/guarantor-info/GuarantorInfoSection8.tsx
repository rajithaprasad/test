...
interface GuarantorInfoSection8Props {
  formData: FormData;
...
const GuarantorInfoSection8 = ({ formData, updateFormData, onNext, onPrevious }: GuarantorInfoSection8Props) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-red-900 to-red-600 bg-clip-text text-transparent mb-3">
          Guarantor Information
        </h2>
        <p className="text-lg text-red-600">Section 8: Bridge Loan - Acquisition</p>
      </div>

      <div className="bg-red-50 rounded-xl p-6 border border-red-100">
        <h3 className="text-xl font-semibold text-red-800 mb-6">Guarantor Details - Section 8</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="floating-input relative">
            <input
              ...
              onChange={(e) => updateFormData({ netWorth: parseInt(e.target.value) || 0 })}
              placeholder=" "
              required
              className="w-full p-4 border-2 border-gray-200 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg"
            />
            <label htmlFor="netWorth" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              Net Worth ($) *
            </label>
          </div>
...
export default GuarantorInfoSection8;
