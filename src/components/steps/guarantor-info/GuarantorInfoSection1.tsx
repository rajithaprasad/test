...
interface GuarantorInfoSection1Props {
  formData: FormData;
...
const GuarantorInfoSection1 = ({ formData, updateFormData, onNext, onPrevious }: GuarantorInfoSection1Props) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
      ...
      <div className="text-center">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-green-900 to-green-600 bg-clip-text text-transparent mb-3">
          Guarantor Information
        </h2>
        <p className="text-lg text-green-600">Section 1: DSCR - Refinance - LTR</p>
      </div>

      <div className="bg-green-50 rounded-xl p-6 border border-green-100">
        <h3 className="text-xl font-semibold text-green-800 mb-6">Guarantor Details - Section 1</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="floating-input relative">
            <input
              type="number"
              id="investmentPropertiesOwned"
              value={formData.investmentPropertiesOwned || ''}
              onChange={(e) => updateFormData({ investmentPropertiesOwned: parseInt(e.target.value) || 0 })}
              placeholder=" "
              className="w-full p-4 border-2 border-gray-200 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg"
            />
            <label htmlFor="investmentPropertiesOwned" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
...
export default GuarantorInfoSection1;
