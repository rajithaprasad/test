...
interface ScenarioSpecificInfo6Props {
  formData: FormData;
...
const ScenarioSpecificInfo6 = ({ formData, updateFormData, onNext, onPrevious }: ScenarioSpecificInfo6Props) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600">Section 6</p>
      </div>

      <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
        <h3 className="text-xl font-semibold text-blue-800 mb-6">Section 6</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex items-center p-4 bg-white/50 backdrop-blur-sm border-2 border-gray-200 rounded-xl hover:border-emerald-200 focus-within:border-emerald-500 transition-all duration-300">
            <input
              type="checkbox"
              id="tearDown"
              checked={formData.tearDown || false}
              onChange={(e) => updateFormData({ tearDown: e.target.checked })}
              className="w-6 h-6 text-emerald-600 rounded focus:ring-emerald-500"
            />
            <label htmlFor="tearDown" className="ml-2 text-lg text-gray-700">
              Tear Down
            </label>
          </div>
...
export default ScenarioSpecificInfo6;
