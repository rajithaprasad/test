...
interface ScenarioSpecificInfo7Props {
  formData: FormData;
...
const ScenarioSpecificInfo7 = ({ formData, updateFormData, onNext, onPrevious }: ScenarioSpecificInfo7Props) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600">Section 7</p>
      </div>

      <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
        <h3 className="text-xl font-semibold text-blue-800 mb-6">Section 7</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="floating-input relative">
            <input
              ...
              className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${errors.originalPurchasePrice ? 'border-red-500' : 'border-gray-200'}`}
            />
          ...
          <div className="floating-input relative">
            <input
              ...
              className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${errors.originalPurchaseDate ? 'border-red-500' : 'border-gray-200'}`}
            />
          ...
          <div className="floating-input relative">
            <input
              ...
              className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${errors.mortgagePayoff ? 'border-red-500' : 'border-gray-200'}`}
            />
          ...
          {showMortgagePayoffDate && (
            <div className="floating-input relative">
              <input
                ...
                className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${errors.mortgagePayoffDate ? 'border-red-500' : 'border-gray-200'}`}
              />
          ...
          <div className="floating-input relative">
            <input
              type="number"
              id="constructionAmountCompleted"
              value={formData.constructionAmountCompleted || ''}
              onChange={(e) => updateFormData({ constructionAmountCompleted: e.target.value })}
            ...
            />
            <label htmlFor="constructionAmountCompleted" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
          ...
          <div className="flex items-center p-4 bg-white/50 backdrop-blur-sm border-2 border-gray-200 rounded-xl hover:border-emerald-200 focus-within:border-emerald-500 transition-all duration-300">
            <input
              type="checkbox"
              id="currentlyVacant"
              checked={formData.currentlyVacant || false}
              onChange={(e) => updateFormData({ currentlyVacant: e.target.checked })}
              className="w-6 h-6 text-emerald-600 rounded focus:ring-emerald-500"
            />
            <label htmlFor="currentlyVacant" className="ml-2 text-lg text-gray-700">
              Currently Vacant
            </label>
          </div>
          ...
          <div className="md:col-span-2 floating-input relative">
            <textarea
              ...
              className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${errors.scopeOfWork ? 'border-red-500' : 'border-gray-200'}`}
              rows={4}
            />
          ...
          {[...Array(3)].map((_, index) => (
            <div key={index} className="md:col-span-2 floating-input relative">
              <textarea
                id={`comparable${index}`}
...
export default ScenarioSpecificInfo7;
