...
interface ScenarioSpecificInfo9Props {
  formData: FormData;
...
const ScenarioSpecificInfo9 = ({ formData, updateFormData, onNext, onPrevious }: ScenarioSpecificInfo9Props) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.planToRepay) {
      newErrors.planToRepay = 'Plan to Repay is required.';
    }

    if (!formData.reasonForBridge) {
      newErrors.reasonForBridge = 'Reason for Bridge is required.';
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600">Section 9</p>
      </div>

      <div className="bg-blue-50 rounded-xl p-6 border border-blue-100">
        <h3 className="text-xl font-semibold text-blue-800 mb-6">Section 9</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="floating-input relative">
            <input
              type="number"
              id="rehabAmountCompleted"
              value={formData.rehabAmountCompleted || ''}
              onChange={(e) => updateFormData({ rehabAmountCompleted: e.target.value })}
            ...
            />
            <label htmlFor="rehabAmountCompleted" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              Rehab Completed ($)
            </label>
          </div>
          ...
          <div className="floating-input relative">
            <select
              ...
              onChange={(e) => updateFormData({ planToRepay: e.target.value })}
              className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${
                errors.planToRepay ? 'border-red-500' : 'border-gray-200'
              }`}
            >
              <option value="">Select Plan to Repay</option>
              <option value="Sell">Sell</option>
              <option value="Refinance">Refinance</option>
            </select>
            <label htmlFor="planToRepay" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              Plan to Repay
            </label>
            {errors.planToRepay && <p className="text-red-500 text-xs mt-1">{errors.planToRepay}</p>}
          </div>

          <div className="floating-input relative md:col-span-2">
            <textarea
              id="reasonForBridge"
              value={formData.reasonForBridge || ''}
              onChange={(e) => updateFormData({ reasonForBridge: e.target.value })}
              className={`w-full p-4 border-2 rounded-xl bg-white/50 backdrop-blur-sm focus:border-emerald-500 focus:bg-white focus:outline-none transition-all duration-300 text-lg ${
                errors.reasonForBridge ? 'border-red-500' : 'border-gray-200'
              }`}
            />
            <label htmlFor="reasonForBridge" className="absolute left-4 top-4 text-gray-500 transition-all duration-300 pointer-events-none bg-white px-1 -mt-2 text-sm">
              Reason for Bridge
            </label>
            {errors.reasonForBridge && <p className="text-red-500 text-xs mt-1">{errors.reasonForBridge}</p>}
          </div>
...
export default ScenarioSpecificInfo9;
